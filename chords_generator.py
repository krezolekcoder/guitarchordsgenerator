...
class ChordsGenerator:
    def __generate_chromatic_scale(self, root_note):
        ...
        if root_note in chromatic_scale_flats and root_note in chromatic_scale_sharps:
            ...
            chromatic_scale = chromatic_scale_sharps[chromatic_scale_flats.index(root_note) : ] + chromatic_scale_sharps[: chromatic_scale_flats.index(root_note)]
        else :
    ...
    def get_chord_type(self,chord_notes):
        ...
        if(interval == 3):
            return "{} Minor".format(chord_notes[0])
        elif(interval == 4):
            return "{} Major".format(chord_notes[0])
        else:
            return "other chord " 
    ...
    def generate_chord(self, chord_root_note, chord_type):
        ...
        # in order to generate major chord we first need to generate the major scale of the root note of that chord 
            ...
            # generated_chord = 
            chord_major_scale = self.generate_major_scale(chord_root_note)
            generated_chord = chord_major_scale[::2][0:3]
        ...
        # and then get the second chord of that major scale which is minor  
            ...
            major_scale_root_note = chromatic_scale[-2]

            chord_major_scale = self.generate_major_scale(major_scale_root_note)

            generated_chord = chord_major_scale[1::2][0:3]
